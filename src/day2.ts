...
function parseInput1(input: string) {
  return input
    ...
    .map(
      (row) =>
        row.split(' ').map((value) => shapesByKey[value as ShapeKey]) as [
          Shape,
          Shape
        ]
    );
...
function parseInput2(input: string) {
  return input.split('\n').map((row) => {
    ...
    return {
      opponentShape: shapesByKey[value1 as ShapeKey],
      outcome: value2 as Outcome,
    };
  });
}
...
function getRoundScore1([opponentShape, myShape]: [Shape, Shape]) {
  ...
  const outcomeScore = isVictory(opponentShape, myShape)
    ? outcomeScores.victory
    : outcomeScores.loss;
